...
//
        case 1:
            puts("Case 1 LOGIN\n");
            if(Writeline(c_socket, buffer, strlen(buffer)) < 0) { 		//Sending the operation number to the server
                puts("[-]Send error.\n");
                exit(1);
            }
        ...
        scanf_psw1:
            ...
            Writeline(c_socket, buffer, strlen(buffer));		//Sending username and password to the server


            if(Readline(c_socket, buffer, MAX_DIM - 1) <= 0) {		//Receiving ack from server
...
//
        ...
        scanf_pswconf:
            ...
            if(strcmp(passwordC, password) != 0) {
                ...
                sprintf(buffer, "%s;%s\n", username, password);
                if(Writeline(c_socket, buffer, strlen(buffer)) <= 0) { 	//Sending username and password to server
                    puts("[-]Send error.\n");
                    exit(1);
                }
            }

            if(Readline(c_socket, buffer, strlen(buffer)) < 0) {		//Receive ack from server
                puts("[-]Error in receiving data.\n");
                exit(1);
            }
...
//
    ...
    while(1) {
        if(Readline(c_socket, buffer, MAX_DIM - 1) < 0) {		//Receiving rows and seat number from server
            puts("[-]Error in receiving data.\n");
            exit(1);
        } else {
...
//
            case 1:
                ...
                if(Readline(c_socket, buffer, MAX_DIM - 1) < 0) { 	//Receive the number of the reserved seats
                    puts("[-]Error in receiving data.\n");
                    exit(1);
                }
                ...
                for(i = 0; i < reserved; i++) {
                    if(Readline(c_socket, buffer, MAX_DIM - 1) <= 0){ 	//Receive the coordinates of the reserved seats
                        puts("[-]Error in receiving data.\n");
                        exit(1);
                    }
...
//
            ...
            scanf_retry2:
                ...
                if(Writeline(c_socket, buffer, strlen(buffer)) <= 0) {	//Sending to server how many seats user wants to reserve
                    puts("[-]Send error.\n");
                    exit(1);
                }
            ...
            scanf_retry_seats:
                ...
                if(Writeline(c_socket, unModBuff, strlen(unModBuff)) <= 0) {	//Send reservation to server
                    puts("[-]Send error.\n");
                    exit(1);
                }

                if(Readline(c_socket, buffer, MAX_DIM - 1) <= 0) {	//Receive ack from server
                    puts("[-]Error in receiving data.\n");
                    exit(1);
                }
                ...
                if(!strcmp(buffer, "success")) {
                    ...
                    if(Readline(c_socket, buffer, MAX_DIM - 1) <= 0) {
                        puts("[-]Receive failed\n");
                        exit(1);
                    }
